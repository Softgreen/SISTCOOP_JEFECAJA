...
/* jshint -W098 */
angular.module('organizacion').controller('Organizacion.TransaccionInterna.BovedaCaja.BuscarController',
  function ($scope, $state, toastr, AGENCIA, SGDialog, CajaService, SessionService, BovedaService, VoucherService) {
    ...
    $scope.loadTransaccionEnviadas = function () {
      BovedaService.getTransaccionBovedaCajaEnviadas(AGENCIA.id).then(
        function (response) {
    ...
    $scope.loadTransaccionRecibidas = function () {
      BovedaService.getTransaccionBovedaCajaRecibidas(AGENCIA.id).then(
        function (response) {
